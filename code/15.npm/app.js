...
//실행하는 명령어의 매개변수를 받아 workingDir로 설정하여 하위폴더로 video, captuer폴더 생성
const folder = process.argv[2]; //코드재활용 + 폴더 생성해준다
const workingDir = path.join(process.cwd(), folder); //경로확인
const videoDir = path.join(workingDir, 'video'); //만들어질 주소. join 스트링이므로 주소를 만들어 오는 것
const captureDir = path.join(workingDir, 'capture');

if (!fs.existsSync(workingDir)) fs.mkdirSync(workingDir); //코드재활용 + 폴더 생성해준다
...
function isImageFile(file){
  ...
  if(fileObject.ext == '.png'||fileObject.ext == '.jpg') return true; //.png라면 true
}
